...
if(!is_dir("$camera_images")) {
  mkdir("$camera_images", 0777, true);
  chmod("$camera_images", 0777);
}
    ...
    if($img != ''){
        ...
        $success = file_put_contents("$camera_images/$file", $decoded);
        ...
        echo "start processing recognition...<br>";

        ob_start();
        $command = 'python recognize_faces_image.py' . $file;
        passthru($command);
        $output = ob_get_clean(); 

        echo "finished processing recognition...";
